...
interface LearnedDao {
    ...
    @Query("DELETE FROM bigrams WHERE previous = :key OR next = :key")
    fun deleteBigramsFor(key: String)
    @Query("DELETE FROM trigrams WHERE first = :key OR second = :key OR next = :key")
    fun deleteTrigramsFor(key: String)
